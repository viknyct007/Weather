...
def get_knyc_6h():
    ...
    try:
        ...
        for f in data.get("features", []):
            v = f.get("properties", {}).get("temperature", {}).get("value")
            if v is not None:
                vals.append(temp_f(v))
        return {"high": max(vals) if vals else None, "low": min(vals) if vals else None}
    except Exception as e:
        print("KNYC history error:", e)
...
<div class="card"><div class="title">Actual KNYC — Past 6 Hours</div><div class="history"><div class="hist"><div class="l">HIGH</div><div class="v" id="hi">—</div></div><div class="hist"><div class="l">LOW</div><div class="v" id="lo">—</div></div></div></div>
...
try{
 ...
 document.getElementById('hi').textContent=t(d.past6Hours.high);document.getElementById('lo').textContent=t(d.past6Hours.low);
